...
pub fn print_engine_report(
    ...
    let events_deadlettered =
        store.event_deadlettered_count();

    let total_attempts =
        store.attempt_history.len() as u64;

    let retried =
        total_attempts.saturating_sub(events_created);
    ...
    let delivered_pct =
        if events_created > 0 {
            (events_delivered as f64 * 100.0)
                / events_created as f64
        } else {
            0.0
        };

    let deadlettered_pct =
        if events_created > 0 {
            (events_deadlettered as f64 * 100.0)
                / events_created as f64
        } else {
            0.0
        };

    let end_to_end_latencies_ms: Vec<f64> =
        store
            .domain_events
            .values()
            .filter_map(|event| {
                event.final_state_at.map(|final_state| {
                    duration_to_ms(
                        final_state.duration_since(
                            event.created_at
                        )
                    )
                })
    ...
    let http_call_durations_ms: Vec<f64> =
        store
            .attempt_history
            .iter()
            .map(|attempt| {
                duration_to_ms(
                    attempt.completed_at.duration_since(
                        attempt.started_at
                    )
                )
            })
    ...
    let e2e_stats =
        summarize(&end_to_end_latencies_ms);

    let http_stats =
        summarize(&http_call_durations_ms);

    let invariant_report =
        store.verify_invariant();

    let all_passed =
        invariant_report.all_passed();
